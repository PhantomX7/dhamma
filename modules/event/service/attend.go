...
func (s *service) Attend(ctx context.Context, eventID uint64, req request.EventAttendRequest) (eventAttendance entity.EventAttendance, err error) {
	...
	card, err := s.cardRepo.FindOneByField(ctx, "code", req.CardCode)
	if err != nil {
		return
	}

	follower, err := s.followerRepo.FindByID(ctx, card.FollowerID)
	if err != nil {
		return
	}
